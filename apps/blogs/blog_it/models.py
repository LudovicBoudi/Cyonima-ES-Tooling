...
class ITArticle(models.Model):
    ...
    class Meta:
        verbose_name = 'Article IT'
        verbose_name_plural = 'Articles IT'
    ...
    def display_id(self):
        return f"IT-{self.year}-{self.sequence_number:04d}"

    def save(self, *args, **kwargs):
        if not self.pk:
            self.year = timezone.now().year
            last = ITArticle.objects.filter(year=self.year).order_by('-sequence_number').first()
            self.sequence_number = (last.sequence_number + 1) if last else 1
        super().save(*args, **kwargs)


class ITArticleAttachment(models.Model):
    article = models.ForeignKey(ITArticle, on_delete=models.CASCADE, related_name='attachments')
    file = models.FileField(upload_to='blog/it/%Y/%m/')
