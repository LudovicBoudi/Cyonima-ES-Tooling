...
from .models import DirectionArticle, DirectionArticleAttachment
...
def article_list(request):
    ...
    all_tags = set()
    for t in DirectionArticle.objects.filter(status='publie').values_list('tags', flat=True):
        for tag_name in (t or '').split(','):
            name = tag_name.strip()
            if name:
                all_tags.add(name)
    return render(request, 'blogs/article_list.html', {
        'articles': page, 'blog_title': 'Blog Direction Générale',
        'list_url': 'dg_blog_list', 'create_url': 'dg_blog_create',
        'detail_url': 'dg_blog_detail', 'edit_url': 'dg_blog_edit',
        'can_write': user_can_write, 'delete_url': 'dg_blog_delete',
...
def article_detail(request, article_id):
    ...
    articles = DirectionArticle.objects.filter(status='publie')
    return render(request, 'blogs/article_detail.html', {
        'article': article, 'articles': articles, 'blog_title': 'Blog Direction Générale',
        'list_url': 'dg_blog_list', 'create_url': 'dg_blog_create',
        'detail_url': 'dg_blog_detail', 'edit_url': 'dg_blog_edit',
        'can_write': can_write(request.user),
        'content_type': 'dg_blog.DirectionArticle',
    })
...
def article_create(request):
    ...
    if request.method == 'POST':
        ...
        if attachment:
            DirectionArticleAttachment.objects.create(article=article, file=attachment, filename=attachment.name)
        create_notification(request.user, f"Nouvel article : {article.display_id()}", f"Article publié dans le blog Direction Générale : {article.title}", link='')
    ...
    return render(request, 'blogs/article_form.html', {
        'page': None, 'blog_title': 'Blog Direction Générale',
        'list_url': 'dg_blog_list', 'create_url': 'dg_blog_create',
        'detail_url': 'dg_blog_detail', 'edit_url': 'dg_blog_edit',
    })
...
def article_edit(request, article_id):
    ...
    if request.method == 'POST':
        ...
        return redirect('dg_blog_detail', article_id=article.id)
    return render(request, 'blogs/article_form.html', {
        'page': article, 'blog_title': 'Blog Direction Générale',
        'list_url': 'dg_blog_list', 'create_url': 'dg_blog_create',
        'detail_url': 'dg_blog_detail', 'edit_url': 'dg_blog_edit',
    })
...
def article_delete(request, article_id):
    ...
    if request.method == 'POST':
        ...
        return redirect('dg_blog_list')
    return render(request, 'blogs/article_confirm_delete.html', {'article': article, 'list_url': 'dg_blog_list'})
